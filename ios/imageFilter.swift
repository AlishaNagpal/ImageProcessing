...
class RNImageFilter: NSObject {
  ...
  func deleteFolder (_ fileUrl: String) -> Void{
    do {
      ...
      let fileURLs = try FileManager.default.contentsOfDirectory(at: filterFolder,includingPropertiesForKeys: nil,options: [.skipsHiddenFiles, .skipsSubdirectoryDescendants])
...
//      guard let callbak:Any = self.callBackURL! else {return}
//      callback([["base64":callbak]])
